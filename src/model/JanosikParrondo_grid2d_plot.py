...
rd = pd.read_csv(script_path + "/data/"+exp_desc+"_"+str(x_vals[0])+"agents"+".zip")


# data from the experiement
for no_agents in x_vals[1:]:
    print(no_agents)
    rd = rd.append(pd.read_csv(script_path + "/data/"+exp_desc+"_"+str(no_agents)+"agents"+".zip"),ignore_index=True)
...
#%% ploting
fig = mpl.figure.Figure(figsize=(7,6.25))
for i,curr_policy in enumerate(default_policies):
    for j,curr_eps in enumerate(default_eps_vals):
        ...
        plot_desc = str(curr_policy)+ ", " + r"$\delta$="+str(curr_eps)
        axs.grid(alpha=0.5,ls=':')
        
        axs.plot(x_vals_dense,gini_data,'k-.')
        ...
        for b in ["matthew", "strongmatthew", "antimatthew", "strongantimatthew" ]:
            ...
            axs.text(10, 0.44, plot_desc, rasterized=False, usetex=True)
            ...
            axs.set_yticks(np.arange(0, 0.51, step=0.2))
...
print("[INFO] Saving","plots/"+ exp_desc +".pdf")
